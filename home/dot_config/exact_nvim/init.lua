...
do
  ...
  -- Review a diff the way a GitHub PR shows it: a file panel on the left, the
  -- diff side-by-side on the right. `<Tab>`/`<S-Tab>` walk the file list.
  --
  -- `main...HEAD` (three dots) diffs against the merge base, so commits that
  -- landed on main after the branch started stay out of the review — same rule
  -- GitHub uses for a PR's "Files changed" tab.
  vim.pack.add { gh 'sindrets/diffview.nvim' }
  require('diffview').setup {}

  vim.keymap.set('n', '<leader>gd', '<cmd>DiffviewOpen<cr>', { desc = '[G]it [D]iff (uncommitted)' })
  vim.keymap.set('n', '<leader>gb', '<cmd>DiffviewOpen main...HEAD<cr>', { desc = '[G]it [B]ranch review' })
  vim.keymap.set('n', '<leader>gc', '<cmd>DiffviewOpen HEAD~1<cr>', { desc = '[G]it last [C]ommit' })
  vim.keymap.set('n', '<leader>gl', '<cmd>DiffviewFileHistory<cr>', { desc = '[G]it [L]og of this branch' })
  vim.keymap.set('n', '<leader>gf', '<cmd>DiffviewFileHistory %<cr>', { desc = '[G]it history of this [F]ile' })
  ...
  require('which-key').setup {
    ...
    spec = {
      ...
      { '<leader>w', group = '[W]orkspace (repo tabs)' },
