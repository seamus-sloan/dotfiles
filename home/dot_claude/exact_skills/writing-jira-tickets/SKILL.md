...
- `/writing-jira-tickets include the region in the export filename, under PROJ-201`
  — drafts and files a ticket under the PROJ-201 epic.
...
- Determine the **epic** and **project**. If the user named an epic (e.g. PROJ-201), use it
...
- Infer the title pattern from siblings. In this user's epics it is almost always:
  ...
  **`<Component/Feature> - <Title Case summary>`**
  ...
  `Reporting - Randomize Sample Data for Demo Mode`.
...
- Use `createJiraIssue` (contentFormat markdown). Resolve the site's cloudId with
  `getAccessibleAtlassianResources` on first use — don't hardcode it.
